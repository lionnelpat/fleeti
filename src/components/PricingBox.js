...
const PricingBox = ({color="#0BA1C1",title,prix,unit,time,text,text2,features}) => {
    ...
    let handleMouseOut = () => {
        setBorder({top: 5,others:0.5})
    }

    return (
       <Grid.Column style={{display: "flex",alignItems: "center",justifyContent:"center"}}>
           <div onMouseLeave={handleMouseOut} onMouseEnter={handleMouseIn} style={{width: "90%", border: `solid ${color} 0.5px`, borderLeftWidth: border.others,borderRightWidth: border.others, borderBottomWidth: border.others , display: "inline-block",borderTopWidth: border.top, borderRadius: 15,textAlign:"center", padding: 30 }}>
