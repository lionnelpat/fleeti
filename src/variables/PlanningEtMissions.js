export const planningEtMissions =
{
    data: [
        {
            img: "pem1",
            title: "Organisez vos<br/> équipes",
            text: "Via ce module, vous aurez une vue en temps<br/> réel de la disponibilité de vos chauffeurs et<br/> véhicules pour organiser le travail de vos<br/> équipes de manière plus efficiente. "
        },
        {
            img: "pem2",
            title: "Impliquez vos<br/> collaborateurs",
            text: "Grâce à notre application mobile, vous<br/> pouvez communiquer directements avec vos<br/> employés qui pourront quant à eux vous<br/> remonter des données terrain en temps réel"
        },
        {
            img: "pem3",
            title: "Gérez vos missions",
            text: "Grâce à notre planning intuitif de gestion<br/> des missions, vérifiez où en sont vos<br/> collaborateurs, quels véhicules sont utilisés<br/> et lesquels sont indisponibles, Modifiez en<br/> temps réel le planning de tournée"
        },
    ],
    title3: "La meilleure façon d’organiser le travail de vos chauffeurs et de vos véhicules",
    img2: "big_pem",
    data2: [
        {
            title: "Utilisez notre agenda",
            text: "Notre agenda interactif et simple d’utilisation vous permet d’organiser le travail de vos chauffeurs selon la disponbilité des véhicules. Vous aurez une vue en temps réel de la gestion de votre flotte en fonction des missions du jour. "
        },
        {
            title: "Analysez votre organisation",
            text: "Voyez à quelle fréquence les véhicules ou chauffeurs sont utilisés sur une période de temps donnés. Vous obtiendrez des rapports sur les temps de livraison, les missions réalisées et les plannings de votre flotte."
        },
        {
            title: "Faîtes la chasse aux documents inutiles",
            text: "Centralisez l’ensemble de vos documents (bons de livraison, réception...) sur notre application mobile dédiée à vos chauffeurs. "
        },

    ]

}




