export const fuelManagement =
{
    data: [
        {
            img: "fm1",
            title: "Suivez en temps réel<br/> votre consommation",
            text: "Vérifiez où, quand et combien de carburant<br/> a été inséré dans le réservoir. Assurez-vous<br/> que tous vos achats ont bien été insérés<br/> dans le réservoir. "
        },
        {
            img: "fm2",
            title: "Planifiez vos budgets",
            text: "Recevez des analyses détaillées des<br/> volumes de carburant consommé pour<br/> mieux préparer vos budgets."
        },
        {
            img: "fm3",
            title: "Fixez des objectifs d’économies",
            text: "Un système de contrôle efficace du<br/> carburant permet en moyenne d’économiser<br/> 15% sur les coûts totaux de carburant. La<br/> mise en place des sondes et du suivi associé<br/> sont généralement remboursés en 3-4 mois. "
        },
    ],
    title3: "Détectez facilement les anomalies de consommation",
    img2: "big_fm",
    data2: [
        {
            title: "Contrôlez votre réservoir",
            text: "Notre interface de suivi en temps réel de votre réservoir vous permet de contrôler chacune des opérations qui l’impactent. Notez les chauffeurs et véhicules en fonction de leurs consommations. "
        },
        {
            title: "Prévenez les vols de carburant",
            text: "Sachez où, quand et combien de carburant a été siphonné du véhicule. Recevez des alertes en temps réel afin de raccourcir la prise de décision. Notre module de suivi carburant vous aide à identifier les vols et de fait réduire l’occurence de ces événements. "
        },
        {
            title: "Ne laissez pas la place à l’improvisation",
            text: "Les coûts de carburant sont trops importants pour se contenter d’analyse de consommation moyenne. Nos outils vous permettent de prendre des décisions éclairées grâce à une analyse précise des données récoltées. L’impact est immédiat sur vos coûts opérationnels. "
        },

    ]

}




